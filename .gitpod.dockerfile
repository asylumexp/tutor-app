...
RUN curl https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add - && \
    ...
    apt-get update && \
    apt-get -y install build-essential dart libkrb5-dev gcc make gradle android-tools-adb android-tools-fastboot openjdk-8-jdk apt install libgtk-3-dev cmake && \
    apt-get clean && \
